...
public:
	...
	using hash_function_type = Hash;
	using hashed_type = decltype(std::declval<Hash>()(std::declval<Key>()));
	using next_smaller_qf = templated_qfilter_seq<Key, remainder_bits+1, Hash>;
...
private:
	...
	{
		while (table[pos.table].is_shifted(pos.cell))
		{
			--pos;
		}
	...
	{
		while (!table[pos.table].is_empty(pos.cell))
		{
			++pos;
		}
	...
	void insert_and_shift(entry_type entry, entry_pos& insert_pos)
	{
		...
		while (!table[end_pos.table].is_empty(end_pos.cell))
		{
			++end_pos;
		}
	...
	entry_pos grow_cluster(next_bigger_qf& target, entry_pos cluster_start)
	{
		...
		auto read = [&]()
		{
			...
			cell_type_trgt::entry_set_remainder(target_entry,
              cell_type::entry_remainder(source_entry) & target_remainder_mask);
		};
...
public:
	...
	qf::InsertResult insert_hash(const hashed_type& hashed) override
	{
		...
		const bool run_already_existing = table[q_pos.table].is_occupied(
            q_pos.cell);
		table[q_pos.table].set_occupied(true, q_pos.cell);
