...
// utils/ is provided by the DySECT include in bumpribbon
...
public:
	...
	InsertResult(std::uint64_t cluster_length) : data(cluster_length & cluster_length_mask) {}
	InsertResult(failed_operation_t) : data(failed_op_mask) {}

	operator bool() const { return successful(); }

	bool successful() const { return !static_cast<bool>(data & failed_op_mask); }
	std::uint64_t cluster_length() const { return data & cluster_length_mask; }

	void set_cluster_length(std::uint64_t cluster_length) { data = cluster_length & cluster_length_mask; }
...
public:
	...
	ContainsResult(failed_operation_t) : data(ContainsResultEnum::FAILED_OPERATION) {}
...
{
	const std::uint64_t fingerprint = hash >> (64 - quotient_bits - remainder_bits);
...
// Recursion case
template <size_t min, size_t I, size_t... Ns>
struct seq_gen<min, I, Ns...>
{
	using type = typename seq_gen<min, I - 1, I, Ns...>::type;
};

// Recursion abort
template <size_t min, size_t... Ns>
struct seq_gen<min, min, Ns...>
{
...
template<class T>
std::string bitstring(T val, size_t bits = sizeof(T) * 8)
{
	std::string str;
...
auto create_handle(QuotientFilter& filter)
{
	if constexpr (QuotientFilter::uses_handle)
	{
		return filter.get_handle();
	}
