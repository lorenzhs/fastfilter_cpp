#ifndef XOR_FILTER3_XOR_FILTER3_H_
#define XOR_FILTER3_XOR_FILTER3_H_
...
class XorFilter3 {
  ...
  size_t arrayLength;
  size_t blockLength;
  FingerprintStorageType *fingerprints0;
  FingerprintStorageType *fingerprints1;
  FingerprintStorageType *fingerprints2;
 ...
 public:
  explicit XorFilter3(const size_t size) {
    hasher = new HashFamily();
    this->size = size;
    this->arrayLength = 32 + 1.23 * size;
    this->blockLength = arrayLength / 3;
    fingerprints0 = new FingerprintStorageType(blockLength);
    fingerprints1 = new FingerprintStorageType(blockLength);
    fingerprints2 = new FingerprintStorageType(blockLength);
  }

  ~XorFilter3() {
    delete fingerprints0;
    delete fingerprints1;
    delete fingerprints2;
    delete hasher;
  }
  ...
  size_t SizeInBytes() const { return fingerprints0->getByteCount() * 3; }
};
...
Status XorFilter3<ItemType, FingerprintType, FingerprintStorageType, HashFamily>::AddAll(
    ...
    for (int i = reverseOrderPos - 1; i >= 0; i--) {
        ...
        fp[change] = fingerprints0->mask(xor2);
    }
    fingerprints0->bulkSet(fp, blockLength);
    fingerprints1->bulkSet(fp + blockLength, blockLength);
    fingerprints2->bulkSet(fp + blockLength * 2, blockLength);
...
template <typename ItemType, typename FingerprintType,
          typename FingerprintStorageType, typename HashFamily>
Status XorFilter3<ItemType, FingerprintType, FingerprintStorageType, HashFamily>::Contain(
    ...
    /*
    uint32_t bp0 = reduce(r0, blockLength) * fingerprints0->getBitsPerEntry();
    uint32_t bp1 = reduce(r1, blockLength) * fingerprints0->getBitsPerEntry();
    uint32_t bp2 = reduce(r2, blockLength) * fingerprints0->getBitsPerEntry();
    uint32_t w0 = *((uint32_t*) (fingerprints0->data + (bp0 >> 3)));
    uint32_t w1 = *((uint32_t*) (fingerprints1->data + (bp1 >> 3)));
    uint32_t w2 = *((uint32_t*) (fingerprints2->data + (bp2 >> 3)));
    f ^= (w0 >> (bp0 & 7)) ^ (w1 >> (bp1 & 7)) ^ (w2 >> (bp2 & 7));
    */
    const size_t bcf = fingerprints0->byteCountFloor;
    uint32_t ab0 = reduce(r0, bcf);
    uint32_t ab1 = reduce(r1, bcf);
    uint32_t ab2 = reduce(r2, bcf);
    uint64_t w0 = *((uint64_t*) (fingerprints0->data_raw + ab0));
    uint64_t w1 = *((uint64_t*) (fingerprints1->data_raw + ab1));
    uint64_t w2 = *((uint64_t*) (fingerprints2->data_raw + ab2));

    uint32_t bp0 = reduce(r0, blockLength) * fingerprints0->getBitsPerEntry();
    uint32_t bp1 = reduce(r1, blockLength) * fingerprints0->getBitsPerEntry();
    uint32_t bp2 = reduce(r2, blockLength) * fingerprints0->getBitsPerEntry();
    uint32_t shift0 = bp0 + 24 - (ab0 * 8);
    uint32_t shift1 = bp1 + 24 - (ab1 * 8);
    uint32_t shift2 = bp2 + 24 - (ab2 * 8);

    f ^= (w0 >> shift0) ^ (w1 >> shift1) ^ (w2 >> shift2);

    return fingerprints0->mask((ItemType)f) == 0 ? Ok : NotFound;
}

template <typename ItemType, typename FingerprintType,
          typename FingerprintStorageType, typename HashFamily>
std::string XorFilter3<ItemType, FingerprintType, FingerprintStorageType, HashFamily>::Info() const {
  std::stringstream ss;
  ss << "XorFilter3 Status:\n"
     << "\t\tKeys stored: " << Size() << "\n";
  return ss.str();
}
}  // namespace xorfilter3
#endif  // XOR_FILTER3_XOR_FILTER3_H_
